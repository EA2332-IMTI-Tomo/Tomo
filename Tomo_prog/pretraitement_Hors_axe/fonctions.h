...
void holo2TF_UBorn(unsigned char *holo1, nbCplx *TF_UBorn,Var2D dimROI,Var2D decalROI, Var2D dim2DHA, Var2D coinHA,size_t NumAngle, double *masque);
...
void lire_bin(string chemin, double resultat[], short int precision, int NbPix);
...
void TF2Dcplx2(nbCplx *entree, nbCplx *fft, Var2D dim, fftw_plan p);
...
void SAV2(double *var_sav, int NbPix2D, std::string chemin_str, enum PRECISION precision, char options[]);
void SAV_Re(nbCplx *var_sav,  int NbPix2D, char *chemin, enum PRECISION precision, char options[]);
void SAV_Re2(nbCplx *var_sav,  int NbPix2D,  std::string chemin, enum PRECISION precision, char options[]);
...
void SAV_Tiff2D_Im(nbCplx *var_sav, char *chemin, int dim);
