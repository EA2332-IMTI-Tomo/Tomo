...
void holo2TF_UBorn(unsigned char *holo1, nbCplx *TF_UBornTot,Var2D dimROI,Var2D decalROI, Var2D dim2DHA, Var2D coinHA, size_t NumAngle, double *masque)
{
        ...
        for(pixel=0; pixel<NbPixROI2d; pixel++) {
                holo[pixel].Re=(double)holo1[pixel]*masque[pixel];
        }
       // #pragma omp barrier
       // SAV_Re(holo, NbPixROI2d, "/home/tomo/Projet_tomo/Tomo_Imagesholo.bin", FLOAT,"a+b");
...
//auto start_time = chrono::high_resolution_clock::now();
     ...
     //   cout << "temps 1er circshift : "<<chrono::duration_cast<chrono::microseconds>(end_time - start_time).count()/10 << " ms"<<endl;;

       // SAV_Re(holo_shift, NbPixROI2d, "/home/tomo/Projet_tomo/Tomo_Images/holo_shift.bin", FLOAT,"a+b");
       //
      ...
      // t_init_TF=clock();
        TF2Dcplx(holo_shift,TF_Holo,dimROI);
                 //SAV_Re(TF_Holo, NbPixROI2d, "/home/aziz/Projet_tomo/Tomo_Images/TF_holo.bin", FLOAT,"a+b");
               // t_fin_TF=clock();
...
//copie pour passer en complexe...
    ...
    //SAV_Re(copie_obj2D_B,NbPts2D,"/home/tomo/Projet_tomo/Tomo_Imagescopie_objetB.bin",FLOAT,"wb");
...
for(int cpt=0;cpt<NbPts2D;cpt++){
   ...
   // SAV_Re(spectCorr,NbPts2D,"/home/tomo/Projet_tomo/Tomo_ImagesSpectCorrRE.bin",FLOAT,"wb");
    //SAV_Re(Corr,NbPts2D,"/home/tomo/Projet_tomo/Tomo_ImagesCorr.bin",FLOAT,"wb");
    return decal2D_I;
}
...
void SAV_Re2(nbCplx *var_sav, int NbPix2D, string chemin, enum PRECISION precision, char options[])
{
        size_t taille=NbPix2D;
        FILE *pfichier_ID=NULL;
        pfichier_ID= fopen(chemin.c_str(), options);

        //printf("identifiant fichier=%i",pfichier_ID);
        if(pfichier_ID==NULL)
                cout<<"Erreur d'ouverture du fichier "<<chemin<<endl;
        ...
        case DOUBLE: //64 bit
                ...
                for(unsigned int cpt=0; cpt<taille; cpt++) {
                        ...
                        fwrite(&tampon,sizeof(tampon),1,pfichier_ID);
                }
                break;
        case FLOAT://32 bits float
...
void SAV_Re(nbCplx *var_sav, int NbPix2D, char *chemin, enum PRECISION precision, char options[])
{
        size_t taille=NbPix2D;
        FILE *pfichier_ID=NULL;
        pfichier_ID= fopen(chemin, options);
...
void charger_image2D(unsigned char* phasei, string imgFile, Var2D coin,Var2D taille)
{
        ...
        //Monimage.write("/home/tomo/Projet_tomo/Tomo_Imagestest.bmp");
...
void TF2Dcplx_INV(nbCplx *fft_entree, nbCplx *objet, Var2D dim)
{
        ...
        p=fftw_plan_dft_2d( dim.x,  dim.y, in, out,FFTW_BACKWARD, FFTW_ESTIMATE);
           //p=fftw_plan_dft_2d( dim.x,  dim.y, in, out,FFTW_BACKWARD,  FFTW_USE_WISDOM);
        fftw_execute(p); /* repeat as needed */
...
///SAV tf phase
//SAV(TfPhaseMod2pi, NbPixROI2d, "/home/tomo/Projet_tomo/Tomo_ImagesIDP/TfPhaseMod2pi/TfPhaseMod2pi", CHAR,"a+b");
///SAV delta
//SAV(dephasageCarre, NbPixROI2d, "/home/tomo/Projet_tomo/Tomo_ImagesIDP/deltaCarre/deltaCarre", CHAR,"a+b");
///sav phase
//SAV(phaseMod2pi, NbPixROI2d, "/home/tomo/Projet_tomo/Tomo_ImagesIDP/phase/phase", CHAR,"a+b");
///sav txModulation
//SAV(txModulFrange, NbPixROI2d, "/home/tomo/Projet_tomo/Tomo_ImagesIDP/txModulation/txModulation", CHAR,"a+b");
///-------
...
void SAV_Tiff2D2(double *var_sav, string chemin, int dim)
{
...
double moyenne(double *entree,unsigned int NbPixImg)
{
    double som_moy=0;
    for(int pixel=0; pixel<NbPixImg; pixel++) {
        som_moy=som_moy+entree[pixel];
    }
    double moy_ref=som_moy/NbPixImg;
    return moy_ref;
    }

void charge_ref(double *ampli_ref, string CheminModRef,int NbPix, Var2D dimROI, Var2D coinROI, Var2D dim2DHA){
    unsigned char* mod_ref=new unsigned char[NbPix]; //reservation reference  pour une "tentative" de correction de la ref
    Image image_ref;
    image_ref.read(CheminModRef);

    if(image_ref.rows()!=IMAGE_DIMY)
    cout<<"Erreur : la largeur de la référence ("<<image_ref.rows()<<") est différente de la largeur camera ("<<IMAGE_DIMY<<")"<<endl;
    ...
    if(image_ref.rows()==0){
        ...
        image_ref.crop(Geometry(dimROI.x,dimROI.y,coinROI.x,coinROI.y));
        char numDim[3+1];
        string nomDim;
        sprintf(numDim,"%03i",2*dim2DHA.x);
        nomDim=numDim;
        nomDim=nomDim+"x"+numDim;
        //cout<<"numDim-------------------------------------------=="<<nomDim<<endl;
        image_ref.resize(nomDim);
        image_ref.getPixels(0,0,dim2DHA.x,dim2DHA.y);
        image_ref.writePixels(GrayQuantum, mod_ref);
        //int i, currentImageWidth, currentImageHeight;

        for(size_t cpt=0;cpt<NbPix;cpt++)
        ampli_ref[cpt]=sqrt(mod_ref[cpt]);
    }
...
void charge_refOpenCV(double *ampli_ref, string CheminModRef,int NbPix, Var2D dimROI, Var2D coinROI, Var2D dim2DHA){

    Mat mod_ref = imread(CheminModRef, 0);
    if(mod_ref.rows!=IMAGE_DIMY)
    cout<<"Erreur : la largeur de la référence ("<<mod_ref.rows<<") est différente de la largeur camera ("<<IMAGE_DIMY<<")"<<endl;


    //cv::Rect myROI(dimROI.x,dimROI.y,coinROI.x,coinROI.y);
  //  cv::Rect myROI(1024,1024,0,0);
  //  Mat mod_ref_crop(dimROI.x,dimROI.y,CV_8U);
    //mod_ref(myROI).copyTo(mod_ref_crop);

 // imshow("Ampli ref crop 1024",mod_ref_crop);
  //  waitKey(0);
    if(mod_ref.rows==0){
        ...
        Mat mod_ref_mini;
        Size size_2DHA(dim2DHA.x,dim2DHA.y);
        resize(mod_ref,mod_ref_mini,size_2DHA,INTER_CUBIC);
       // imshow("Ampli ref retaille à 220x220",mod_ref_mini);
       // waitKey(0);

        for(size_t x=0;x<dim2DHA.x;x++){
                  for(size_t y=0;y<dim2DHA.y;y++){
                    size_t cpt=dim2DHA.x*y+x;;//conversion cpt 1D coord 2D
                    ampli_ref[cpt]=sqrt((double)(mod_ref_mini.at<uchar>(y,x)));//openCv->tableau, et calcul ampli=srqt(module)
                }
        }
    }
}
