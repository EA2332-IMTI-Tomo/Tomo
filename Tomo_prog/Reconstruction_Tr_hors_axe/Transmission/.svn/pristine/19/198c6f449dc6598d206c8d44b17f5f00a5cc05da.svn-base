...
double* specSynth(double *spectreSynRe, double *spectreSynIm, int NbPlanZ, Var2D dimFin);
...
///############################################
	    ...
	    int cpt=0;
	    float precision;
	    for(int z=cpt_max; z < cpt_max+1; z++)
	    {
...
double* specSynth(double *spectreSynRe, double *spectreSynIm, int NbPlanZ, Var2D dimFin)
{
    ...
    for(int y=0; y<dimFin.y; y++)
    {
        for(int x=0; x<dimFin.x; x++)
        {
            ...
            {
                if(Amp[index].at<double>(y,x) > Amp[index_max].at<double>(y,x))
                {
                    index_max = index;
                }
                else
            ...
            spectreSynIm[cpt]=Re[index_max].at<double>(y,x);
        }
    }
    ...
    SAV(planSynRe, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/PlanSynMax.bin", FLOAT,"a+b");
