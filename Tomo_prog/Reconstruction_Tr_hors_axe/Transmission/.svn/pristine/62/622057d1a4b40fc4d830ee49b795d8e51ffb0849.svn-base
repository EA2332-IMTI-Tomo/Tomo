...
int main(int argc, char *argv[])
{
        ...
        int DIMX_CCD2=dimCCD.x,  DIMY_CCD2=dimCCD.y,
            NbPixROI2d=dimCCD.x*dimCCD.y;//nombre de pixel (pour le tableau 2D)
        ///Chemin fichiers-------------------------------------
        char* input_dir="/opt/resultat2012/session12010401_micrometre/";
        string Dossier_acquiz=input_dir , CheminModRef=Dossier_acquiz+"mod_ref.bmp";
        string NumSession="session12010401-record",NomHoloPart1=Dossier_acquiz+NumSession;
...
//valeur controlant l'exclusion de certains centres
        ...
        Var2D coin {550,140}; //valeur du coin pour la découpe
        if(coin.x+dimCCD.x>1280 || coin.y+dimCCD.y>1082)//largeur : 740, hauteur : 574.
...
///--selections des hologrammes par numéro
        ...
        cout <<"Dim finale : " << round(4*zoom*n1*Tps*DIMX_CCD2/(G*lambda)*NA/n1) << " pixels cube\n";
        int   centres_exclus=0,   nb_proj=0;
        const int N_tab=dim_final*dim_final*dim_final;//Nombre de pixel dans l'espace final (indice max du tableau 3D+1)
...
// 	unsigned char* noir_camera=new unsigned char[NbPixROI2d];
        ...
        nbCplx *plan=new nbCplx[NbPixROI2d], nbCplx *plan_shift=new nbCplx[NbPixROI2d];
///variables utilisées pour la TF2D
//Avant Crop à NXMAX;
        nbCplx *fft_tmp=new nbCplx[NbPixROI2d];
//Après Crop à NXMAX;
        nbCplx *fft=new nbCplx[NbPixROI2d], nbCplx *fft_shift=new nbCplx[4*NXMAX*NYMAX];
...
///----------__--__-----début de la grosse boucle sur les angles/////////////////////////
        char charAngle[4+1];
        double coef_IDP=0;
...
///calcul du champ complexe  par IDP---------------------------------------------
                        ...
                        SAV_Re(fft_shift,4*NXMAX*NXMAX, "/home/mat/tomo_test/Tf2d_reel.bin", FLOAT,"a+b");
                        ...
                        for(int cpt=0; cpt<(4*NXMAX*NYMAX); cpt++) {
                                fft_reel_shift_norm[cpt]=(fft_shift[cpt].Re*max_part_reel+fft_shift[cpt].Im*max_part_imag)/max_module;
                                fft_imag_shift_norm[cpt]=(fft_shift[cpt].Im*max_part_reel-fft_shift[cpt].Re*max_part_imag)/max_module;
                        }
                        ...
                        if((xm0*xm0+ym0*ym0)>rayon_inf) { //inutile?
                                ...
                                retroPropag(spectre3D_Re,spectre3D_Im, sup_redon, dim_final,fft_reel_shift_norm,fft_imag_shift_norm, posSpec, decal, NMAX, rayon);
                        }//fin else xm0_limite
       ...
       // delete[] plan_reel, plan_imag;
        delete[] plan, plan_shift;
        delete[] fft_tmp, fft, fft_shift;
        delete[] fft_reel_shift_norm, fft_imag_shift_norm, fft_module_shift;
        delete[] masque;

///fin exportation plan corrigé supredon
        ...
        SAV(centre, 4*NXMAX*NYMAX, "/home/mat/tomo_test/centres.bin", UINT,"a+b");
        cout<<"centres exportés"<<endl;
        SAV(sup_redon, N_tab, "/home/mat/tomo_test/sup_redon_C.bin", UINT,"a+b");
...
//TF3D(spectre3D_Re_shift,spectre3D_Im_shift,res_reel, res_imag);
        ...
        delete[] spectre3D_Re_shift;//libertation de la memoire d'entree
       ...
       // ecrire_rapport(NXMAX,rayon,Rf,DIMX_CCD2,coin.x,coin.y,sizeof(precision)*4,Dossier_acquiz,nb_proj,n1,NA,Tp,G);
