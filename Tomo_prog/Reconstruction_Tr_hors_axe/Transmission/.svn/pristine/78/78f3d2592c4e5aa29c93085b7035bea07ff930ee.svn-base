...
void 
readLoop_binaryVolume(double* reel_arc, double* imag_arc, unsigned short int* sup_redon, int* centre, \
...
{    
  ...
  const int N = g_dimx_ccd * g_dimy_ccd;
  ...
  **/
  ...
  double* readImage;
  ARRAY_ALLOC(readImage, image_dimx * image_dimy ,double); 
 ...
 // BOUCLE DE LECTURE
  for (int I = 0; I < NbAngle; I++)
    {
      ...
      CHRONO_START(load2d_time);
      assert(fread(readImage, sizeof(double), image_dimx * image_dimy, fp) != EOF);
      extract_subImage(readImage, plan_reel, image_dimx, image_dimy, coin_x, coin_y, g_dimx_ccd, g_dimy_ccd);
      assert(fread(readImage, sizeof(double), image_dimx * image_dimy, fp) != EOF);
      extract_subImage(readImage, plan_imag, image_dimx, image_dimy, coin_x, coin_y, g_dimx_ccd, g_dimy_ccd);
      CHRONO_STOP(load2d_time);
  ...
  //************************************************************************************************************************
      ...
      circshift_ccd2(plan_imag, plan_imag_shift, g_dimx_ccd, g_dimy_ccd, g_dimx_ccd/2, g_dimy_ccd/2);
      ...
      else
	{
	  ...
	  centre[xmi*2*Nxmax+ymi]=I; //cpt_angle;
