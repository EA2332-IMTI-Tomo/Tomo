...
if(coin_x+DIMX_CCD2>740 || coin_y+DIMY_CCD2>574)//largeur : 740, hauteur : 574.
...
char Chemin[]="/opt/resultats_2010/modulateur/Rf_0.833/session10120801/session10120801-record";
...
printf("K : %f\n", K );
printf("Rayon %f \n",rayon);
cout<<"Taille théorique des pixels :"<<tailleTheoPixel<<" nm"<<endl;
//printf("DIMX_CCD2 %i \n",DIMX_CCD2);
...
NXMAX_Rf=n1*Tps*DIMX_CCD2/(G*lambda)*NA/n1;
int
NYMAX=NXMAX,
...
unsigned char* holo1Re=new unsigned char[N];
unsigned char* holo2Re=new unsigned char[N];
unsigned char* holo3Re=new unsigned char[N];
unsigned char* holo4Re=new unsigned char[N];

unsigned char* holo1Im=new unsigned char[N];
unsigned char* holo2Im=new unsigned char[N];
unsigned char* holo3Im=new unsigned char[N];
unsigned char* holo4Im=new unsigned char[N];
...
for(int cpt_angle=premier_plan;cpt_angle<premier_plan+NbAngle;cpt_angle=cpt_angle+SautAngle)//boucle sur tous les angles
{
    ...
    if(test_existence!=NULL)
    {
		...
		///décalage de phase
		holo1Re=charger_image2D(holo1Re,1,Chemin,cpt_angle, coin_x, coin_y,DIMX_CCD2,DIMY_CCD2);
        /*	if(holo1Re==NULL)
		{
		printf("Le fichier n'existe pas\n");
		}
        else{*/
		holo2Re=charger_image2D(holo2Re,2,Chemin,cpt_angle, coin_x, coin_y,DIMX_CCD2,DIMY_CCD2);
		holo3Re=charger_image2D(holo3Re,3,Chemin,cpt_angle, coin_x, coin_y,DIMX_CCD2,DIMY_CCD2);
		holo4Re=charger_image2D(holo4Re,4,Chemin,cpt_angle, coin_x, coin_y,DIMX_CCD2,DIMY_CCD2);



        ///Calcul du déphasage réel (méthode de Carré), et du taux de modulation
         double *tan_alpha=new double[N];//\delta=2*alpha, bref vaudrait mieux trouver 45° et tan alpha autour de 1
         double *phaseMod2pi=new double[N];//calcul de la phase [2pi] de l'hologramme, méthode de carré
         double somme_tan_alpha=0;
         int nb_tan=0;

         double somme_ecart_carre=0;
         int compteurIDP_ok=0;
         double txModulFrange=0;
         //calcul de la valeur du saut de phase
         for(int pixel=0;pixel<N;pixel++)
         		{
         			phased1[pixel]=(double)holo1Re[pixel];
         			phased2[pixel]=(double)holo2Re[pixel];
         			phased3[pixel]=(double)holo3Re[pixel];
         			phased4[pixel]=(double)holo4Re[pixel];
         			///démodulation
         				phased1=circshift(phased1, DIMX_CCD2,DIMY_CCD2,DIMX_CCD2/2,DIMY_CCD2/2);


                       // TF2D(plan_reel_shift,plan_imag_shift,fft_reel_tmp,fft_imag_tmp,DIMX_CCD2,DIMY_CCD2);

          			tan_alpha[pixel]=sqrt(fabs((3*(phased2[pixel]-phased3[pixel])-(phased1[pixel]-phased4[pixel]))/((phased1[pixel]-phased4[pixel])+(phased2[pixel]-phased3[pixel]))));//formule de Carré
          			dephasageCarre[pixel]=2*atan(tan_alpha[pixel])*180/3.14;
          			phaseMod2pi[pixel]=atan(sqrt((3*(phased2[pixel]-phased3[pixel])-phased1[pixel]+phased4[pixel])*(phased1[pixel]-phased4[pixel]+phased2[pixel]-phased3[pixel]))/(phased2[pixel]+phased3[pixel]-phased1[pixel]-phased4[pixel]));
         			...
         			{
          			somme_tan_alpha=tan_alpha[pixel]+somme_tan_alpha;
                    txModulFrange=2*sqrt((pow(phased4[pixel]-phased2[pixel],2)+pow(phased1[pixel]-phased3[pixel],2)))/(phased1[pixel]+phased2[pixel]+phased3[pixel]+phased4[pixel])+txModulFrange;
         			nb_tan++;
         			}
         		}
///SAV delta
...
///-------
        ...
        txModulFrange=txModulFrange/nb_tan;
         for(int pixel=0;pixel<N;pixel++)
         		{
          			if(tan_alpha[pixel]<1.4&&tan_alpha[pixel]>0.6)//la valeur de la tangente doit valoir environ tan(0.5*pi/2)=1, on élimine les valeurs aberrantes>10
         			{
...
///Fin méthode de carré
		...
		for(int pixel=0;pixel<N;pixel++)
		{
			plan_reel[pixel]=((double)holo1Re[pixel]-(double)holo3Re[pixel])*(masque[pixel]);
			plan_imag[pixel]=((double)holo4Re[pixel]-(double)holo2Re[pixel])*(masque[pixel]);
            //plan_reel[pixel]=(((double)holo1Re[pixel]/(double)reference[pixel])-((double)holo3Re[pixel]/(double)reference[pixel]));//*(masque[pixel]);
            //plan_imag[pixel]=(((double)holo4Re[pixel]/(double)reference[pixel])-((double)holo2Re[pixel]/(double)reference[pixel]));//*(masque[pixel]);
       ...
       //  multiplier_masque(fft_imag_shift, cache_jumeau, NXMAX, t_mask, xmi, ymi);//effacer objet (pour voir!)
         //  multiplier_masque(fft_reel_shift, cache_jumeau, NXMAX, t_mask, xmi, ymi);
          ...
          // multiplier_masque(fft_reel_shift, cache_jumeau, NXMAX, t_mask, 39, 99);
            ...
            multiplier_masque(fft_imag_shift, cache_jumeau, NXMAX, t_mask, NXMAX, NXMAX);
...
/////////////////////////////////////////////////////////////////////////////////////
 ...
 printf("coucou1\n");
	...
	ecrire_rapport(NXMAX,rayon,Rf,K,DIMX_CCD2,coin_x,coin_y,precision_exportation,Chemin,nb_proj,n1,NA,Tp,G);
//
	for(int cpt=0;cpt<(N3D);cpt++)
		{
