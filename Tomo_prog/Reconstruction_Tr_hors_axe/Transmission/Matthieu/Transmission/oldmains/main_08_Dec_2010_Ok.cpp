...
double multiplier_masque(double image[],double masque[], int centreX, int centreY);
...
char Chemin[]="/opt/resultats_2010/modulateur/Rf_0.833/session10120801/session10120801-record";// /opt2/resultats2009/lyon_aout_09/muscles/session09082401-record";

/*/opt2/resultats2009/LYON/session09111701
int coin_x_tmp;
int coin_y_tmp;*/
int premier_plan=1;
int Num_Angle_final=511;//
// int sans_signal=0;
...
if(dimx_ccd2==512)
{
masque=rempli_tableau("/home/mat/Mat/Matlab/kmasq_512x512_30.bmp", 0, 0,dimx_ccd2,dimy_ccd2);
printf("masque 512x512\n");
}
if(dimx_ccd2==256)
{
masque=rempli_tableau("/home/mat/Mat/Matlab/kmasq_256x256.bmp", 0, 0,dimx_ccd2,dimy_ccd2);
printf("masque 256x256\n");
}

//taille totale du masque en pixel pour elimination objet jumeau;
const int t_mask=30;
cache_jumeau=rempli_tableau("/home/mat/Mat/Matlab/k_30x30.bmp", 0, 0,t_mask,t_mask);
...
//cout<<"valeur Réel_cptJumo_max="<<fft_reel_shift_norm[cptJumo_max]<<endl;
	////////////////////////////////////Virer l'objet jumeaux
		...
		{ jumeau_elimine++;
        fft_imag_shift_norm[]=multiplier_masque(double fft_imag_shift_norm[],double cache_jumeau[], int centreX, int centreY)
...
*/
            ///sauver tf après écrasemrent jumeau et ordre zero
...
multiplier_masque(double image[],double masque[], int centreX, int centreY)
{
	for(int x=centreX-t_mask/2;x<centreX+t_mask/2;x++)
	{
		...
		{			//attention le masque repasse du coté gauche lorsque le jumeau touche le bord droit! A corriger
			...
			image_masquee[cptj]=image[cptj]*masque[t_mask*(y-centreY+t_mask/2)+(x-centreX+t_mask/2)]/255;
		}
	}
