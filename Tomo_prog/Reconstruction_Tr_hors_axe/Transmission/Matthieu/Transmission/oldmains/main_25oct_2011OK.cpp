...
char Chemin[]="/opt/resultat_2011/session11021802_pollenPN/session11021802-record";
...
tailleTheoPixel=Tp/(G/Rf)*pow(10,9);//Pour info
...
const int
NXMAX=round(n1*Rf*Tps*DIMX_CCD2/(G*lambda)*NA/n1+1),//NXMAX=round(rayon*NA/n1)=round(rayon*sin theta_max);
NXMAX_Rf=(n1*Tps*DIMX_CCD2/(G*lambda)*NA/n1+1);
int
NYMAX=NXMAX,
...
for(int cpt_angle=premier_plan;cpt_angle<premier_plan+NbAngle;cpt_angle=cpt_angle+SautAngle)//boucle sur tous les angles
{
    ...
    if(test_existence!=NULL)
    {
		...
		///décalage de phase
		holo1=charger_image2D(holo1,1,Chemin,cpt_angle, coin_x, coin_y,DIMX_CCD2,DIMY_CCD2);
        /*	if(holo1Re==NULL)
		{
		printf("Le fichier n'existe pas\n");
		}
        else{*/
		holo2=charger_image2D(holo2,2,Chemin,cpt_angle, coin_x, coin_y,DIMX_CCD2,DIMY_CCD2);
		holo3=charger_image2D(holo3,3,Chemin,cpt_angle, coin_x, coin_y,DIMX_CCD2,DIMY_CCD2);
		holo4=charger_image2D(holo4,4,Chemin,cpt_angle, coin_x, coin_y,DIMX_CCD2,DIMY_CCD2);
        ...
        for (int y = -NYMAX; y < NYMAX; y++)//on balaye l'image 2D en x , origine (0,0) de l'image au milieu
			{
				...
				for (int x = -NXMAX; x < NXMAX; x++)//on balaye l'image 2D en y, centre au milieu
				{
					...
					{
                        float z_carre=r2-x*x-y_carre; //altitude au carré des données
                        float z=round(sqrt(z_carre)-zm0);
                        int altitude=round((z+dv2s2rf)*dv1xdv0rf);
						k=round((-xm0+x+dv0s2rf)+(-ym0+y+dv1s2rf)*dv0rf+altitude);//indice du tableau 1D du volume 3D
