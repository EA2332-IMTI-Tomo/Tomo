...
int main(int argc, char *argv[])
{
  ...
  char Dossier_acquiz[]="/home/hui/maniptomo/reflexion_acquisition/2013070801/";
  char NumSession[]="i";
  ...
  */
  ...
  double *plan_amp_norm=new double[dimFin.x*dimFin.y];
  ...
  double  *spectreC_ampli=new double[dimFin.x*dimFin.y];

  double *spectre2D_ReCoup=new double[dimInit.x*dimInit.y];
  double *spectre2D_ImCoup=new double[dimInit.x*dimInit.y];
  ...
  double *distFit= new double[dimFin.x*dimFin.y];
  double *distSharp= new double[dimFin.x*dimFin.y];
  double *distRef=new double[dimFin.x*dimFin.y];
  double *distRecal=new double[dimFin.x*dimFin.y];
  ...
  double *spectreI_ampli_shift=new double[dimFin.x*dimFin.y];
  double *spectreC_reel_shift=new double[dimFin.x*dimFin.y];
  ...
  double *dist = new double[image_size];
  double *phaseMod1pi=new double[image_size];
  double *phaseMod1piWrap=new double[image_size];
  double *phaseunwrap=new double[image_size];
  double *phaseMod1piFit=new double[image_size];
  double *distMod1pi=new double[image_size];
  PIXEL *Pixel = new PIXEL[image_size];
  EDGE *Edge = new EDGE[No_of_Edges];
  Mat masqueC=imread("vlsi_0604.jpg",0);
  Mat masqueF=imread("masque_120.bmp",0);
  double *masqueFft= new double[dimInit.x*dimInit.y];
  ///Copier matrice de opencv vers double///
  for(int y=0; y<masqueF.rows; y++)
    {
      for(int x=0; x<masqueF.cols; x++)
        {
	  int cpt=y*masqueF.cols+x;
	  masqueFft[cpt]=masqueF.at<uchar>(y,x);
        }
    }
  ...
  for(int cpt_angle=premier_plan;cpt_angle<premier_plan+NbAngle;cpt_angle=cpt_angle+SautAngle)//boucle sur tous les angles
      ...
      if(test_existence!=NULL)
	{
	  ...
	  memset(supRedonPlan, 0, dimFin.x*dimFin.y*8);
	  ...
	  //SAV(fft_reel_tmp, N, "/home/hui/maniptomo/IDP/champ_2d/TF_reel_avant.bin", FLOAT,"a+b");
	  ...
	  ///######################################################Spectre rectangle à rond###################################################
	  for(int cpt=0;cpt<(dimInit.x*dimInit.y);cpt++)
	    {
	      fft_reel_shift[cpt]=fft_reel_shift[cpt]*masqueFft[cpt];
	      fft_imag_shift[cpt]=fft_imag_shift[cpt]*masqueFft[cpt];
	    }
	  SAV(fft_reel_shift, dimInit.x*dimInit.y, "/home/hui/maniptomo/IDP/champ_2d/TF_reel_apres.bin", FLOAT,"a+b");
	  ...
	  cout<<"Coordonnées speculaire="<<cpt_angle<<endl<<"xc="<<xc<<","<<"yc="<<yc<<","<<"zc="<<zc<<"."<<endl;
	  ...
	  */
	  ...
	  AutofocusGrad(fft_reel_shift_norm, fft_imag_shift_norm, fft_reel_shift_refocal, fft_imag_shift_refocal, dimInit, rayon, CoordSpecI, cpt_angle);
	  //AutofocusMod(fft_reel_shift_norm, fft_imag_shift_norm, fft_reel_shift_refocal, fft_imag_shift_refocal, dimInit, rayon, CoordSpecI, cpt_angle);
	  SAV(fft_reel_shift_refocal, dimInit.x*dimInit.y, "/home/hui/maniptomo/IDP/champ_2d/fft_reel_shift_refocal.bin", FLOAT,"a+b");

	  ///#################################################################################################
	  //Var2D posSpec={xc,yc}, Nmax={NXMAX,NXMAX};
	  //FourierCoupe(spectre2D_ReCoup,spectre2D_ImCoup, fft_reel_shift_refocal,fft_imag_shift_refocal, posSpec, Nmax, rayon);
	  //SAV(spectre2D_ReCoup, dimInit.x*dimInit.y, "/home/hui/maniptomo/IDP/champ_2d/spectre2D_ReCoup.bin", FLOAT,"a+b");
	  ...
	  for(int cpt=0; cpt<dimFin.x*dimFin.y;cpt++)
	    {
	      ...
	      spectreC_ampli[cpt]=sqrt(pow(spectreC_imag[cpt],2)+pow(spectreC_reel[cpt],2));
	    }
	  ...
	  circshift2(spectreI_ampli,spectreI_ampli_shift,dimFin,fftDecal);
	  SAV(spectreI_ampli_shift, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/spectreHolo.bin", FLOAT,"a+b");

	  SAV(spectreC_ampli, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/spectreHoloC.bin", FLOAT,"a+b");
	  ...
	  for(int cpt=0;cpt<(dimFin.x*dimFin.y);cpt++)
	    {
	      //plan_amp_norm[cpt]=sqrt(pow(plan_reel_norm_shift[cpt],2)+pow(plan_imag_norm_shift[cpt],2));  //Utiliser pour les plans non refocalisés
	      plan_amp_norm[cpt]=sqrt(pow(plan_reel_norm[cpt],2)+pow(plan_imag_norm[cpt],2));  //Utiliser pour les plans refocalisés
	    }
	  SAV(plan_amp_norm, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/amplitude2D.bin", FLOAT,"a+b");

	  ///#####################################calcul contraste de speckle####################################################################
	  //ContrSpeckle(plan_amp_norm, dimFin.x, contrSpeckle, masqueC);
	  ...
	  //        recalAmpPha(plan_amp_norm, phaseMod1pi, ampliRef, phaseRef, dimFin, plan_reel_norm_recal, plan_imag_norm_recal, cpt_angle, posDecalAmpli, posDecalPhase, NbAngle);
	  ...
	  ///##########################################calcul phase avant déroulement#########################################################
	  ...
	  phaseWrap(phaseMod1pi, phaseMod1piWrap, dimFin.x);
	  //corrPhase(phaseMod1pi,phaseMod1piFit,dimFin.x, masqueC);   /// compensation aberration
	  for(int cpt=0; cpt<dimFin.x*dimFin.y; cpt++)
	    {
	      distMod1pi[cpt] = phaseMod1pi[cpt]*lambda*pow(10,9)/(4*PI*n1);
	    }
	  SAV(phaseMod1pi, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/phaseMod1pi.bin", FLOAT,"a+b");
	  SAV(phaseMod1piWrap, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/phaseMod1piWrap.bin", FLOAT,"a+b");
	  write_data("/home/hui/maniptomo/IDP/champ_2d/distMod1pi.bin",distMod1pi,image_size);

	  ///#####################################calcul la distance à partir de phase#######################################################
	  for(int cpt=0; cpt<dimFin.x*dimFin.y; cpt++)
	    {
	      dist[cpt] = phaseunwrap[cpt]*lambda*pow(10,9)/(4*PI*n1)/coef;
	    }
	  //corrPhase(dist,distFit,dimFin.x, masqueC);   /// compensation aberration

	  ///exportation distance
	  SAV(dist, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/distance.bin", FLOAT,"a+b");
	  SAV(distFit, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/distanceFit.bin", FLOAT,"a+b");

	  Resharp(distFit, distSharp, dimFin.x);
	  SAV(distSharp, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/distanceSharp.bin", FLOAT,"a+b");
	  ///#########################################corrélation croisée sur distance#######################################################
	  for(int cpt=0;cpt<dimFin.x*dimFin.y;cpt++)
	    {
	      distFit[cpt]=(double)sqrt(pow((distFit[cpt]),2));
	    }
	  ...
	  if(cpt_angle==0)
	    {
	      for(int cpt=0;cpt<dimFin.x*dimFin.y;cpt++)
		{
		  distRef[cpt]=distFit[cpt];
		}
	    }
	  posDecalDist=corr_crois(distRef, distFit, dimFin);
	  //Tlérance sur le décalage. Si décalage supérieure à la tolérance, remttre à zero le décalage trouvé.
	  decal2DGen(distFit, distRecal,dimFin, posDecalDist);
	  SAV(distRecal, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/distanceRecal.bin", FLOAT,"a+b");

	  SAV(supRedonSpec, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/supRedonSpec.bin", FLOAT,"wb");
	  coefSomme=coef+coefSomme;
	  contrSpeckleSomme=contrSpeckleSomme+contrSpeckle;
	  nomfinal++;} //fin de if
      else{
  ...
  delete[] plan_reel, plan_imag, plan_reel_norm, plan_imag_norm, plan_amp_norm, plan_reel_norm_shift, plan_imag_norm_shift;
  delete[] spectreI_imag_shift, spectreI_reel_shift, plan_amp_norm;
  delete[] dist, distFit, distRecal;
  //delete[] fft_reel_shift, fft_imag_shift, fft_module_shift;
  delete[] plan_reel_shift, plan_imag_shift;
  delete[] spectreC_reel, spectreC_imag, spectreI_reel, spectreI_imag;
  ...
  //double *planSynRe= new double[dimFin.x*dimFin.y];
  //double *planSynIm= new double[dimFin.x*dimFin.y];
  ////specSynMax(planSynRe,planSynIm,nomfinal, dimFin);
  //
  ...
  //
  ////double contrSpeckleMoy=contrSpeckleSomme/nomfinal;
  ////cout<<"Moyen de contraste speckle="<<contrSpeckleSomme<<endl;
  //
  //double *planAmpTot= new double[dimFin.x*dimFin.y];
  //for(int cpt=0;cpt<(dimFin.x*dimFin.y);cpt++)
  //{
  //    planAmpTot[cpt]=sqrt(pow(planReTot[cpt],2)+pow(planImTot[cpt],2));  //Utiliser pour les plans refocalisés
  //}
  //ContrSpeckle(planAmpTot, dimFin.x, contrSpeckle);
  //delete[] planAmpTot;
  //
  ...
  double  *spectreI_ampliTot_shift=new double[dimFin.x*dimFin.y];
  for(int cpt=0; cpt<dimFin.x*dimFin.y;cpt++)
    {
      spectreI_ampliTot_shift[cpt]=sqrt(pow(spectreI_imagTot_shift[cpt],2)+pow(spectreI_reelTot_shift[cpt],2));
    }
  SAV(spectreI_ampliTot_shift, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/spectresynth2D.bin", FLOAT,"a+b");
  ...
  memset(distMod1pi, 0, dimFin.x*dimFin.y*8);
  memset(phaseMod1piFit, 0, dimFin.x*dimFin.y*8);
  //phaseUnwrapping(planReTot, planImTot, phaseMod1pi, phaseunwrap, Pixel, Edge, dimFin.x);
  phaseUnwrapping(plan_reel_normMoy, plan_imag_normMoy, phaseMod1pi, phaseunwrap, Pixel, Edge, dimFin.x);//Utiliser pour les plans refocalisés
  //phaseUnwrapping(plan_reel_MoyFin, plan_imag_MoyFin, phaseMod1pi, phaseunwrap, Pixel, Edge, dimFin.x);//Utiliser pour les plans non refocalisés
  //phaseUnwrapping(planSynRe, planSynIm, phaseMod1pi, phaseunwrap, Pixel, Edge, dimFin.x);

  phaseWrap(phaseMod1pi, phaseMod1piWrap, dimFin.x);

  double  *plan_amp_normMoy=new double[dimFin.x*dimFin.y];
  for(int cpt=0;cpt<(dimFin.x*dimFin.y);cpt++)
    {
      //plan_amp_normMoy[cpt]=sqrt(pow(plan_reel_MoyFin[cpt],2)+pow(plan_imag_MoyFin[cpt],2));  //Utiliser pour les plans non refocalisés
      plan_amp_normMoy[cpt]=sqrt(pow(plan_reel_normMoy[cpt],2)+pow(plan_imag_normMoy[cpt],2));  //Utiliser pour les plans refocalisés
    }
  SAV(plan_amp_normMoy, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/amplitudeSyn2D.bin", FLOAT,"a+b");
  //ContrSpeckle(plan_amp_normMoy, dimFin.x, contrSpeckle, masqueC);

  ///___________________________________________________Calcul phase après synthèse2D_________________________________________________
  float coef_syn2D=coefSomme/nomfinal;
  cout<<"coef_syn2D="<<coef_syn2D<<endl;

  corrPhase(phaseMod1piWrap,phaseMod1piFit,dimFin.x, masqueC);
  for(int cpt=0; cpt<dimFin.x*dimFin.y; cpt++)
    {
      distMod1pi[cpt] = phaseMod1piFit[cpt]*lambda*pow(10,9)/(4*PI*n1)/coef_syn2D;
    }
  write_data("/home/hui/maniptomo/IDP/champ_2d/distMod1piSyn2D.bin",distMod1pi,image_size);
  write_data("/home/hui/maniptomo/IDP/champ_2d/phaseMod1piSyn2D.bin",phaseMod1piFit,image_size);
  ...
  double *distSyn2D = new double[dimFin.x*dimFin.y];
  for(int cpt=0; cpt<dimFin.x*dimFin.y; cpt++)
    {
      distSyn2D[cpt] = phaseunwrap[cpt]*lambda*pow(10,9)/(4*PI*n1)/coef_syn2D;
    }
  double *distSyn2DFit= new double[dimFin.x*dimFin.y];
  //corrPhase(distSyn2D,distSyn2DFit,dimFin.x, masqueC);
  SAV(distSyn2D, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/distanceSyn2D.bin", FLOAT,"a+b");
  SAV(distSyn2DFit, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/distanceSyn2D_Fit.bin", FLOAT,"a+b");
  ...
  //delete[] planReTot, planImTot, spectreReTot, spectreImTot, supRedonPlanN;
  //getchar();
