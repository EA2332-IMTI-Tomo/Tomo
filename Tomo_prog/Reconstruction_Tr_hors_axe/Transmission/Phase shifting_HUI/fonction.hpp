...
void phaseWrap(double* WrappedImage, double* UnwrappedImage, int taille);
...
void calculPlan1(unsigned char* holo1, unsigned char* holo2, unsigned char* holo3, unsigned char* holo4, double* plan_reel, double* plan_imag,
                int taille, Var2D coin, char* Fichier_holo, double* masque, unsigned char* mod_ref, double * supRedon, double * supRedon_shift);
...
void ContrSpeckle(double* holo1, int taille, double contrSpeckle, Mat masqueC);
int FourierCoupe(double *spectre2D_ReCoup,double*spectre2D_ImCoup, double *Spectre2D_Re,double *Spectre2D_Im,
                Var2D posSpec,Var2D NMAX, double rayon);
...
void ecrire_rapport(int NXMAX,float rayon,float Rf, float K, int DIMX_CCD2,int coin_x, int coin_y,short int precision_exportation,char* chemin,int nb_proj,float n1,float NA,float Tp, int G)
{
    ...
    fichier_rapport = fopen ("/home/hui/maniptomo/rapport_calcul.txt", "wt") ;
...
*/
/////calcul phase de -PI à PI avec arcSin
//     for(int pixel=0;pixel<taille_x*taille_y;pixel++)
//     {
...
//          			if(sin_phase>0)
//                    {
//                        double racine=sqrt(pow(sin_phase,2)+pow(cos_phase,2));
//                        phaseMod1pi[pixel]=asin(sin_phase/racine);
//                    }
//                    else
//                    {
//                        double racine=sqrt(pow(sin_phase,2)+pow(cos_phase,2));
//                        phaseMod1pi[pixel]=PI-asin(sin_phase/racine);
//                    }
//     }
...
//            {
//                double racine=sqrt(pow(sin_phase,2)+pow(cos_phase,2));
//                phaseMod1pi[pixel]=-asin(sin_phase/racine)-PI;
//            }
//            else if(cos_phase==0)
//            {
...
//        }
//     }
///calcul phase de 0 à 2PI avec arcsin
//     for(int pixel=0;pixel<taille_x*taille_y;pixel++)
//     {
//        double cos_phase=partie_reel[pixel];
//        double sin_phase=partie_imag[pixel];
...
//            {
//                double racine=sqrt(pow(sin_phase,2)+pow(cos_phase,2));
//                phaseMod1pi[pixel]=asin(sin_phase/racine)+2*PI;
//            }
//            else if(cos_phase<0)
//            {
//                double racine=sqrt(pow(sin_phase,2)+pow(cos_phase,2));
...
//            if(cos_phase>0)
//            {
//                phaseMod1pi[pixel]=0;if(phaseMod1pi[pixel]<0){cout<<"phaseMod1pi="<<phaseMod1pi[pixel]<<endl;}
//            }
//            else if(cos_phase<0)
//            {
//                phaseMod1pi[pixel]=PI;
...
//        }
//     }
///calcul phase de -pi à PI avec arctan
     for(int pixel=0;pixel<taille_x*taille_y;pixel++)
     {
        double cos_phase=partie_reel[pixel];
        double sin_phase=partie_imag[pixel];
...
///calcul phase de 0 à 2PI avec arctan
//     for(int pixel=0;pixel<taille_x*taille_y;pixel++)
//     {
//        double cos_phase=partie_reel[pixel];
//        double sin_phase=partie_imag[pixel];
...
//            {
//                double argTanPhase=sin_phase/cos_phase;
//                phaseMod1pi[pixel]=atan(argTanPhase)+2*PI;
//            }
//            else if(cos_phase==0)
//            {
//                phaseMod1pi[pixel]=3*PI/2;
...
void phaseWrap(double* WrappedImage, double* UnwrappedImage, int taille)
{
    ...
    for(int cpt=0; cpt<N; cpt++)
    {
        if((WrappedImage[cpt+1]-WrappedImage[cpt])>2*PI)
        {
            UnwrappedImage[cpt+1]=WrappedImage[cpt]-2*PI;
        }
        else if ((WrappedImage[cpt+1]-WrappedImage[cpt])<2*PI)
        {
            UnwrappedImage[cpt+1]=WrappedImage[cpt]+2*PI;
        }
    }
...
void corrplan_I(double *plan_reel, double *plan_imag, double *plan_reel_fit, double *plan_imag_fit, int taille,  Mat masqueC)
{
    ...
    write_dataM("/home/hui/maniptomo/IDP/champ_2d/fond_reel.bin", resultatpolyR);
...
void CalculPoly(Mat& imagebrute,Mat& Masque, Mat& polynome,int deg, bool method)
{
    ...
    //write_dataM("/home/hui/maniptomo/IDP/champ_2d/B.bin", B);
                 ...
                 int cpt_angle, Var2D posDecalAmpli, Var2D posDecalPhase, int NbAngle)
{
    ...
    SAV(phase_recal, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/phaseMod1pi_recal.bin", FLOAT,"a+b");
    ...
    SAV(ampli_recal, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/amplitude2D_recal.bin", FLOAT,"a+b");
    ...
    SAV(plan_reel_recal, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/plan_reel_recal.bin", FLOAT,"a+b");
                ...
                int taille, Var2D coin, char* Fichier_holo, double* masque, unsigned char* mod_ref)
{
    ...
    SAV(plan_reel, taille*taille, "/home/hui/maniptomo/IDP/champ_2d/plan_reel.bin", FLOAT,"a+b");
    delete[] ampli_ref;
}

void calculPlan1(unsigned char* holo1, unsigned char* holo2, unsigned char* holo3, unsigned char* holo4, double* plan_reel, double* plan_imag,
                int taille, Var2D coin, char* Fichier_holo, double* masque, unsigned char* mod_ref, double * supRedonPlanN, double * supRedonPlan)
{
    int N=taille*taille;
    holo1=charger_image2D(holo1,0,Fichier_holo, coin.x, coin.y,taille,taille);
    holo2=charger_image2D(holo2,1,Fichier_holo, coin.x, coin.y,taille,taille);
    ...
    for(int pixel=0;pixel<N;pixel++)
    {
        ...
        {
            //plan_reel[pixel]=((double)holo1[pixel]-(double)holo2[pixel])*(masque[pixel]);
            //plan_imag[pixel]=((double)holo3[pixel]-(double)holo2[pixel])*(masque[pixel]);
            plan_reel[pixel]=((double)holo1[pixel]-(double)holo3[pixel])*(masque[pixel]);
            plan_imag[pixel]=((double)holo4[pixel]-(double)holo2[pixel])*(masque[pixel]);
            //ampli_ref[pixel]= sqrt((double)mod_ref[pixel]);
            //plan_reel[pixel]=(((double)holo1[pixel]-(double)holo3[pixel]))*(masque[pixel])/ampli_ref[pixel];
    ...
    SAV(DataModu, N, "/home/hui/maniptomo/IDP/champ_2d/DataModu.bin", FLOAT,"a+b");
    ...
    SAV(supRedonPlan, size*size, "/home/hui/maniptomo/IDP/champ_2d/supRedonPlan.bin", FLOAT,"wb");
    delete[] ampli_ref;
}
...
int FourierCoupe(double *spectre2D_ReCoup,double*spectre2D_ImCoup, double *Spectre2D_Re,double *Spectre2D_Im,
                Var2D posSpec,Var2D NMAX, double rayon)
{
    int ki_x=posSpec.x,ki_y=posSpec.y;
    double R=10; int xdc, ydc;
    for (int y = -NMAX.y; y < NMAX.y; y++)
    {
        for (int x = -NMAX.x; x < NMAX.x; x++)
        {
            ...
            if(x*x+y*y<R*R)
            {
                xdc=x;
                ydc=y;
                cpt=(ydc+NMAX.y)*2*NMAX.x+xdc+NMAX.x;
                spectre2D_ReCoup[cpt]=Spectre2D_Re[cpt];//(z+ki_z);
                spectre2D_ImCoup[cpt]=Spectre2D_Im[cpt];//(z+ki_z);
            }
            else
            {
                cpt=(y+NMAX.y)*2*NMAX.x+x+NMAX.x;
                spectre2D_ReCoup[cpt]=0;
                spectre2D_ImCoup[cpt]=0;
            }
        }
    }
...
//    double ki_z;
//    //printf("round(rayon*rayon-(xm0)^2-(ym0)^2: %i\n",round(rayon*rayon-(xm0)^2-(ym0)^2));
//    double ki_zcarre = rayon*rayon-ki_x*ki_x-ki_y*ki_y;
//    if(round(ki_zcarre)>-1)
//    {
//        ki_z=sqrt(ki_zcarre);
//        //cout<<"ki_z="<<ki_z<<endl;
//        int NXMAX_CARRE=NMAX.x*NMAX.x;
//        for (int y = -NMAX.y; y < NMAX.y; y++)
//        {//on balaye l'image 2D en y , origine (0,0) de l'image au milieu
//            for (int x = -NMAX.x; x < NMAX.x; x++)
//            {//on balaye l'image 2D en x , origine (0,0) de l'image au milieu
//                int cpt=(y+NMAX.y)*2*NMAX.x+x+NMAX.x;
//                if(x*x+y*y<NXMAX_CARRE)//ne pas depasser l'ouverture
//                {
//                    double z_carre=rayon*rayon-x*x-y*y; //altitude au carré des données
//                    double z=sqrt(z_carre);
//
//                    if(z>50)
//                    {
//                        spectre2D_ReCoup[cpt]=Spectre2D_Re[cpt];//(z+ki_z);
...
void AutofocusGrad(double *fft_shift_normRe, double *fft_shift_normIm, double *sortieRe, double *sortieIm, Var2D dimInit, double rayon, Var2D CoordSpecI, int cpt_angle)
 {
     ...
     int zInf=-10, zSup=10, NbPlanZ=zSup-zInf;
...
///############################################
        ...
        FILE* fichier_objet3DIm = NULL;
  	    fichier_objet3DRe = fopen("/home/hui/maniptomo/IDP/champ_2d/objet3DRe_refocal.bin", "wb");
  	    fichier_objet3DIm = fopen("/home/hui/maniptomo/IDP/champ_2d/objet3DIm_refocal.bin", "wb");
	    int cpt=0;
	    float precision;
        ...
        read_dataM("/home/hui/maniptomo/IDP/champ_2d/objet3DRe_refocal.bin", Re, dimInit.x, NbPlanZ);
        read_dataM("/home/hui/maniptomo/IDP/champ_2d/objet3DIm_refocal.bin", Im, dimInit.x, NbPlanZ);
...
double* specSynMax(double *planSynRe, double *planSynIm, int NbPlanZ, Var2D dimFin)
{
    ...
    read_dataM("/home/hui/maniptomo/IDP/champ_2d/spectreI_reel.bin", Re, dimFin.x, NbPlanZ);
    read_dataM("/home/hui/maniptomo/IDP/champ_2d/spectreI_imag.bin", Im, dimFin.x, NbPlanZ);
    read_dataM("/home/hui/maniptomo/IDP/champ_2d/spectreI_ampli.bin", Amp, dimFin.x, NbPlanZ);
    ...
    SAV(spectreSynRe_shift, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/spectreSynMax.bin", FLOAT,"a+b");

    TF2D_INV(spectreSynRe, spectreSynIm, planSynRe, planSynIm, dimFin.x, dimFin.y);
    SAV(planSynRe, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/PlanSynMaxRe.bin", FLOAT,"a+b");
    SAV(planSynIm, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/PlanSynMaxIm.bin", FLOAT,"a+b");
...
void ContrSpeckle(double* holo1, int taille, double contrSpeckle, Mat masqueC)
{
    ...
    double *masqObj=new double[taille*taille];
    ///Copier matrice de opencv vers double///
    for(int y=0; y<masqueC.rows; y++)
    {
        for(int x=0; x<masqueC.cols; x++)
            {
                int cpt=y*masqueC.cols+x;
                masqObj[cpt]=masqueC.at<uchar>(y,x);
            }
    }
    SAV(masqObj, taille*taille, "/home/hui/maniptomo/IDP/champ_2d/masqObj.bin", FLOAT,"a+b");
    ...
    for(int y=0; y< taille; y++)
    {
        for(int x=0; x< taille; x++)
        {
            ...
            sommeInten=sommeInten+Inten[cpt];
        }

    }    SAV(Inten, taille*taille, "/home/hui/maniptomo/IDP/champ_2d/IntenSpeck.bin", FLOAT,"a+b");
//    for(int pixel=0; pixel< taille*taille; pixel++)
//    {
//        Inten[pixel]=holo1[pixel];
//        sommeInten=sommeInten+Inten[pixel];
//    }
    meanInten=sommeInten/nbPoint;
    ...
    for(int y=0; y< taille; y++)
    {
        for(int x=0; x< taille; x++)
        {
            ...
            if(masqObj[cpt]!=0)
            {
                sommeSigma=sommeSigma+pow(Inten[cpt]-meanInten,2);
            }
        }
...
//    }
    ...
    sigma=sqrt(sommeSigma/(nbPoint-1));
    //double contrSpeckle;
