...
void recalAmpPha(double* amplitude, double* phase, double * ampliRef, double * phaseRef, Var2D dimFin, double* sortie_reel, double* sortie_imag,
                 int cpt_angle, Var2D posDecalAmpli, Var2D posDecalPhase, int NbAngle, double * Gradient);
...
void Autofocus(double *fft_shift_normRe, double *fft_shift_normIm, double *sortieRe_refocal, double *sortieIm_refocal, Var2D dimFin, Var3D dimVol, double rayon, Var2D CoordSpecI);
                 ...
                 int cpt_angle, Var2D posDecalAmpli, Var2D posDecalPhase, int NbAngle, double * Gradient )
{
    ...
    double *ampli_resharp= new double[dimFin.x*dimFin.y];
    ...
    SAV(ampli_recal, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/ampli_norm_recal.bin", FLOAT,"a+b");

    Resharp(ampli_recal, ampli_resharp, dimFin.x);
    SAV(ampli_resharp, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/ampli_norm_resharp.bin", FLOAT,"a+b");

//    ///#######################Cherche Somme Gradient#####################///
//
//    double *ampli1= new double[dimFin.x*dimFin.y];
//    double *ampli2= new double[dimFin.x*dimFin.y];
//
//    Var2D decal1={0,-1};
//    Var2D decal2={-1,0};
//
//    circshift2(ampli_resharp,ampli1,dimFin,decal1);
//    circshift2(ampli_resharp,ampli2,dimFin,decal2);
//
//    double  *Grad=new double[dimFin.x*dimFin.y];
//    double sommeGradient=0;
//    for(int cpt=0;cpt<dimFin.x*dimFin.y;cpt++)
//    {
//        Grad[cpt]=sqrt(pow(ampli_resharp[cpt]-ampli1[cpt],2)+pow(ampli_resharp[cpt]-ampli2[cpt],2));
//        sommeGradient=sommeGradient+Grad[cpt];
//    }
//    //cout<<"Gradient="<<Grad<<endl;
//    double *sommeGrad=new double[NbAngle];
//    for(int cpt=0;cpt<NbAngle;cpt++)
//    {
//        sommeGrad[cpt]=sommeGradient;
//    }
//    Gradient[cpt_angle]=sommeGrad[cpt_angle];
//    cout<<"cpt_angle="<<cpt_angle<<" Gradient="<<Gradient[cpt_angle]<<endl;
//    delete[] ampli1, ampli2;
    ///#######################recal phase#####################///
    ...
    if(cpt_angle==0)
    {
        ...
        //SAV(phaseRef, dimFin.x*dimFin.y, "/home/hui/maniptomo/IDP/champ_2d/phaseREF.bin", FLOAT,"a+b");
    }
    ...
    delete[] ampli_recal, ampli_resharp;
    delete[] phase_recal, plan_reel_recal, plan_imag_recal, spectre_reel, spectre_imag, spectre_module, spectre_reel_norm, spectre_imag_norm;
}
                ...
                int taille, Var2D coin, char* Fichier_holo, double* masque, unsigned char* mod_ref, int cpt_angle)
{
    ...
    for(int pixel=0;pixel<N;pixel++)
    {
        ...
        if(qualiteHolo[pixel]>0.0)
        {
            sommeQholo=sommeQholo+qualiteHolo[pixel];
        }

        if(qualiteHolo[pixel]>0.1)
        {
    ...
    delete[] qualiteHolo,sommeP_reel,sommeP_imag,plan_I42,plan_I13;
    //delete[] supRedonQ;
}
...
void Grad(double* entreeRe, double* entreeIm, double* Gradient, int taille, int NbPlan, int cptPlan)
{
    ...
    SAV(phase, taille*taille, "/home/hui/maniptomo/IDP/SA/phase3D.bin", FLOAT,"a+b");
    ...
    SAV(Grad, taille*taille, "/home/hui/maniptomo/IDP/SA/Grad.bin", FLOAT,"a+b");
...
void Autofocus(double *fft_shift_normRe, double *fft_shift_normIm, double *sortieRe, double *sortieIm, Var2D dimInit, double rayon, Var2D CoordSpecI)
 {
     ...
     double *spectreRe_refocalInc=new double[dimInit.x*dimInit.x];
     double *spectreIm_refocalInc=new double[dimInit.x*dimInit.x];

     double *spectreRe_refocalInc_shift=new double[dimInit.x*dimInit.x];
     double *spectreIm_refocalInc_shift=new double[dimInit.x*dimInit.x];
...
//        }
        ...
        SAV(objet3DReTmp, dimInit.x*dimInit.x*NbPlanZ, "/home/hui/maniptomo/IDP/champ_2d/Objet3D_Re.bin", FLOAT,"a+b");
        ...
        SAV(objet3DRe_refocal, dimInit.x*dimInit.y, "/home/hui/maniptomo/IDP/champ_2d/objet3DRe_refocal0.bin", FLOAT,"a+b");
        ...
        circshift2(spectreRe_refocal, spectreRe_refocalInc, dimInit, plandecal);
        circshift2(spectreIm_refocal, spectreIm_refocalInc, dimInit, plandecal);
        delete[] spectreRe_refocal, spectreIm_refocal;

        Var2D decalI={xm0-dimInit.x/2,ym0-dimInit.x/2};
        circshift2(spectreRe_refocalInc, spectreRe_refocalInc_shift, dimInit, decalI); ///Ajouter les franges d'inclinaision
        circshift2(spectreIm_refocalInc, spectreIm_refocalInc_shift, dimInit, decalI);
        delete[] spectreIm_refocalInc, spectreRe_refocalInc;

        circshift2(spectreRe_refocalInc_shift, sortieRe, dimInit, plandecal);
        circshift2(spectreIm_refocalInc_shift, sortieIm, dimInit, plandecal);
        delete[] spectreRe_refocalInc_shift, spectreIm_refocalInc_shift;
 }
