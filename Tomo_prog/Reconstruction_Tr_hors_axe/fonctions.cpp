...
void holo2TF_UBorn(unsigned char *holo1, nbCplx *TF_UBorn,Var2D dimROI,Var2D decalROI, Var2D dim2DHA, Var2D coinHA)
{
        ...
        for(size_t pixel=0; pixel<NbPixROI2d; pixel++) {
                holo[pixel].Re=(double)holo1[pixel];
        }
        ...
        ///--------Découpe hors axée------------------
        coupeCplx(TF_Holo_centre, TF_UBorn, dimROI, dim2DHA, coinHA);///Découpe à [-Nxmax,+NXmax]
...
{       int fftwThreadInit;
        ...
        fftw_plan p3D;
        clock_t temps_initial = clock();//enclenchement chronometre
        p3D=fftw_plan_dft_3d(dimVol.x, dimVol.y, dimVol.z, in3D, out3D,FFTW_FORWARD, FFTW_EXHAUSTIVE);

         clock_t temps_final = clock ();
         clock_t temps_cpu = (temps_final - temps_initial) * 1e-6;
        printf("temps apres TF3D : %f\n",temps_cpu);
...
std::cout << elapsed.count()/1000000<<"ms" << '\n';
                               ...
                               // double r2=rayon*rayon, kzm0, kzm0_carre = n0*rayon*rayon-kxm0*kxm0-kym0*kym0;
                                ...
                                if(round(kzm0_carre)>-1) {
                                        ...
                                        for (int kdy = -NMAX.y; kdy < NMAX.y; kdy++) { //on balaye l'image 2D en x , origine (0,0) de l'image au milieu
                                                ...
                                                for (int kdx = -NMAX.x; kdx < NMAX.x; kdx++) { //on balaye l'image 2D en y, centre au milieu
                                                        int cpt=(kdy+NMAX.y)*2*NMAX.x+kdx+NMAX.x;//calcul du cpt du tableau 1D de l'image 2D
                                                        if(kdx*kdx+kdy_carre<NMAX_CARRE) { //ne pas depasser l'ouverture numérique pour 1 hologramme
                                                                double kdz_carre=r2-kdx*kdx-kdy_carre; //altitude au carré des données
                                                                ...
                                                                cptPot=(-kxm0+kdx+decal3D.x)+(-kym0+kdy+decal3D.y)*dimVolX+round(altitude);//indice du tableau 1D du volume 3D
                                                                //cout<<"k"<<k<<endl;
...
{       int fftwThreadInit;
        ...
        for(int cpt=0; cpt<(N); cpt++) {
                fft[cpt].Re=out[cpt][0]/N; //division par N^2 pour normaliser la fftw qui n'est pas normalisée
...
///#########lecture propre d'un fichire binaire 3D, connaissant sa taille et son type de données
