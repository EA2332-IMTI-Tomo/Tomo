...
void AcquireImages( PvDevice *aDevice, PvStream *aStream, PvPipeline *aPipeline, coord3D cercle)
{
    ...
    double* fft= new double [1024*1024];
    double* fft_shift= new double [1024*1024];
    double* fft_log= new double [1024*1024];
        ...
        if ( lResult.IsOK() )///Buffer acquis?
        {
            if ( lOperationResult.IsOK() ) ///l'opération finie sans erreur? (time out..)
            {
                ...
                if ( lType == PvPayloadTypeImage )///le buffer contient bien une image? Tout est ok, on traite le flux
                {
                    ...
                    /// Display
                    namedWindow("Histogramme_plan_image", CV_WINDOW_AUTOSIZE);
                    ...
                    circle(fft_mat, Point(512,512),2*cercle.c,(0,0,255),1,8,0);
                    //circle(fft_mat, Point(cercle.a,cercle.b), 3,(128,128128),2,8,0);//centre
